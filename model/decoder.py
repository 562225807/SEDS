...
def build_PEC_decoder(encoder_outputs, encoder_states, multimodal_outputs, lex_emb, embeddings,
                      num_per, person_ids, per_cat_units,
                      num_layers, num_units, cell_type, num_emo, emo_cat, emo_cat_units,
                      emo_int_units, state_pass=True, infer_batch_size=None,
                      attn_num_units=128, target_ids=None, beam_size=None,
                      max_iter=20, is_pretrain=False, dtype=tf.float32, forget_bias=1.0,
                      name="PEC_decoder"):
    # parameter checking
    if infer_batch_size is None:
        txt = "infer_batch_size not specified, infer output will be 'None'."
    ...
    with tf.variable_scope(name):
        ...
        emo_cat_embeddings = tf.Variable(
            ...
            name="emo_cat_embeddings", dtype=tf.float32)
        emo_emb = tf.nn.embedding_lookup(emo_cat_embeddings, emo_cat)

        # create person category embeddings
        per_init = tf.contrib.layers.xavier_initializer()
        per_cat_embeddings = tf.Variable(
            per_init(shape=(num_per, per_cat_units)),
            name="per_cat_embeddings", dtype=tf.float32)
        per_emb = tf.nn.embedding_lookup(per_cat_embeddings, person_ids)
        ...
        memory = encoder_outputs
        mmmemory = multimodal_outputs
        person_lexicons = lex_emb

        cell = PECWrapper(
            cell, memory, mmmemory, person_lexicons, dec_init_states, attn_num_units, num_units, dtype,
            emo_emb, emo_cat, num_emo, emo_int_units, per_emb)
